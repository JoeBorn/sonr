...
import com.smartphonebeuro.sonr.v0.MicSerialListener.ByteReceiver;
...
public class SONRClient extends Service
{
	...
	static private MicSerialListener serialListener = null;
	private ByteReceiver myByteReceiver;
	private int theKeyEvent;
	SONR sonrctx;
	final AudioManager theAudioManager;
	int volume = -1;
	boolean ismuted = false;
	AudioRecord theaudiorecord;
	int bufferSize;
			
	public SONRClient(SONR ctx, AudioRecord ar, int buffsize, final AudioManager am) {
		sonrctx = ctx;
		theAudioManager = am;
		theaudiorecord = ar;
		bufferSize = buffsize;
	...
	public void onCreate() {	
		sonrctx.registerReceiver(clientStopReceiver, new IntentFilter(SONR.DISCONNECT_ACTION));
		myByteReceiver = new SONRByteReceiver();
		serialListener = new MicSerialListener(sonrctx, theaudiorecord, bufferSize, myByteReceiver);
	}
	...
	void sendbroadcast() {
		...
		synchronized(this) {
			...
			sonrctx.sendOrderedBroadcast(i, null);
